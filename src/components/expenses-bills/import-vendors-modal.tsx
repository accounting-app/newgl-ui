...
function downloadSampleFile() {
  ...
  link.href = url;
  link.download = "vendor-import-sample.csv";
  link.click();
  URL.revokeObjectURL(url);
}

// Matches the reference 3-step "Import vendors" wizard. Phase 1, UI only:
// only step 1 (file selection) is real -- picking a file enables Next,
// and the sample-file download is a genuine client-side CSV, since
// that's just static content. Parsing/mapping columns and actually
// creating Vendor records (steps 2-3) is real import logic, not UI, so
// it's deferred along with the rest of the "features little by little".
export function ImportVendorsModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-[var(--color-container-background-primary)]">
      <div className="flex items-center justify-between border-b border-[var(--color-divider-tertiary)] px-6 py-3">
        <h2 className="text-xl font-semibold text-[var(--color-text-global)]">Import vendors</h2>
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h3 className="mb-3 text-lg font-semibold text-[var(--color-text-global)]">First time importing vendors?</h3>
            <ul className="list-disc space-y-1 pl-5 text-sm text-[var(--color-text-primary)]">
              <li>All your vendor information must be in one file</li>
              <li>The top row of your file must contain a header title for each column of information</li>
              <li>
                <span className="font-semibold">Vendor Name</span> is the only required field
              </li>
            </ul>
          </div>
